...
gseaplot1 <- function(dir,files,x){
  ...
  gseaCol=c("#58CDD9","#7A142C","#5D90BA","#431A3D","#91612D","#6E568C","#E0367A","#D8D155",
...
gseaplot2 <- function(dir,files,x){
  files <- paste0(dir,files)
  data=lapply(files, read.delim)                             #读取每个文件
  names(data)=sapply(strsplit(files,'/'),'[',4)
  dataSet=ldply(data, data.frame)
